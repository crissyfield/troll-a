...
const (
	...
	magicZStdFrame          = "\x28\xb5\x2f\xfd" // Magic number of the ZStd frame format (RFC 8478, section 3.1.1)
	magicZStdSkippableFrame = "\x2a\x4d\x18"     // Magic number of the ZStd skippable frame format (RFC 8478, section 3.1.2)
)
...
func decompressBzip2(br *bufio.Reader) (io.ReadCloser, error) {
	// Open BZip2 reader
	dr := io.NopCloser(bzip2.NewReader(br))
